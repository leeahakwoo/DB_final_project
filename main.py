...
def execute_script(sql, params=None):
    conn = get_conn()
    cur = conn.cursor()
...
st.markdown('<div class="sub-text">상품, 창고, 공급자, 입출고 및 재고거래 이력을 통합 관리하는 SQLite 기반 정보시스템</div>', unsafe_allow_html=True)
...
if menu == "대시보드":
    ...
    col3.metric("공급자 수", f"{supplier_count}개")
        ...
        if selected:
            ...
            if not item.empty:
                ...
                with st.form("product_update_form"):
                    ...
                    spec = st.text_input("규격", row["spec"])
                    unit = st.text_input("단위", row["unit"])
                    base_price = st.number_input("기준가격", min_value=0, value=int(row["base_price"]))
                    category = st.text_input("분류", row["category"])
                    status = st.selectbox("상태", ["정상", "중지", "삭제"], index=["정상", "중지", "삭제"].index(row["status"]) if row["status"] in ["정상", "중지", "삭제"] else 0)
...
# =========================
    ...
    selected_category = col2.selectbox("상품분류", ["전체"] + sorted(df["분류"].unique().tolist()))
